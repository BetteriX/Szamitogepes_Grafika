...
/* Az OpnGL ablak minimális szélesség vagy magasság értéke. */
/* Minimum of width or height od the OpenGL window. */
const GLdouble	WORLD_SIZE = 1.0f;
...
void init(GLFWwindow* window) {
	...
	locationMatProjection = glGetUniformLocation(renderingProgram, "matProjection");
	locationMatModelView = glGetUniformLocation(renderingProgram, "matModelView");
...
void framebufferSizeCallback(GLFWwindow* window, int width, int height) {
	...
	window_width = glm::max(width, 1);
	window_height = glm::max(height, 1);

	GLdouble aspectRatio = (GLdouble)window_width / window_height;
	...
	/** Orthographic projekció beállítása, WORLD_SIZE lesz a szélesség és magasság közül a kisebbik. */
	/** Set up orthographic projection, WORLD_SIZE will equal the smaller value of width or height. */
	if (window_width < window_height)
		matProjection = ortho(-WORLD_SIZE, WORLD_SIZE, -WORLD_SIZE / aspectRatio, WORLD_SIZE / aspectRatio, -1.0, 1.0);
	else
		matProjection = ortho(-WORLD_SIZE * aspectRatio, WORLD_SIZE * aspectRatio, -WORLD_SIZE, WORLD_SIZE, -1.0, 1.0);
