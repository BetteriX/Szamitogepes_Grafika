...
char		windowTitle[]	= "Drag and Drop";
...
GLint getActivePoint(vector<glm::vec3> p, GLfloat sensitivity, GLfloat x, GLfloat y) {
	...
	GLfloat		xNorm		= x / ((GLfloat)windowWidth  / 2.0) - 1.0f;
	GLfloat		yNorm		= y / ((GLfloat)windowHeight / 2.0) - 1.0f;
...
void cursorPosCallback(GLFWwindow* window, double xPos, double yPos) {
	...
	if (dragged >= 0) {
		...
		GLfloat	xNorm = xPos / ((GLfloat)windowWidth / 2.0f) - 1.0f;
		GLfloat	yNorm = ((GLfloat)windowHeight - yPos) / ((GLfloat)windowHeight / 2.0f) - 1.0f;
...
void mouseButtonCallback(GLFWwindow* window, int button, int action, int mods) {
	...
	if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS) {
		...
		dragged = getActivePoint(myPoints, 0.1f, x, windowHeight - y);
	}
