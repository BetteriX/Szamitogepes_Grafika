...
GLchar					windowTitle[] = "Parametric Surface";
/** Modellezési mátrixok felvétele. */
/** Modelling matrices. */
mat4					rotateM, scaleM, translateM;
...
static vector<vec3>		surface_vertices;	// vertices of the parametric surface
static vector<GLuint>	surface_indices;	// indices for drawing
...
void generateSurfacePoints(int numHorizontalFaces, int numVerticalFaces) {
	...
	float verticalStep		= radians(360.0f) / numVerticalFaces;
	...
	for (int i = 0; i < numVerticalFaces; ++i) {
		for (int j = 0; j < numHorizontalFaces; ++j) {
			...
			for (int k = 0; k < sizeof(SURFACE_INDICES) / sizeof(SURFACE_INDICES[0]); ++k) {
				ivec2 relIndices = SURFACE_INDICES[k];
...
void initShaderProgram() {
	...
	glBufferData(GL_ARRAY_BUFFER, surface_vertices.size() * sizeof(vec3), surface_vertices.data(), GL_STATIC_DRAW);
...
void computeModelMatrices() {
	/** A tórusz forgatása. */
	/** Rotating the torus. */
	scaleM		= scale(mat4(1.0f), vec3(0.5f));
	translateM	= translate(mat4(1.0f), vec3(0.0f, 0.0f, -3.0f));
	rotateM		= rotate(mat4(1.0f), GLfloat(glfwGetTime() / 2.5f), vec3(0.0f, 1.0f, 0.0f));
	matModel	= translateM * rotateM * scaleM;
}

void computeCameraMatrix() {
	matView = lookAt(cameraPosition, cameraPosition + cameraDirection, cameraUpVector);
}
...
void display(GLFWwindow* window, double currentTime) {
	...
	glDrawElements(GL_TRIANGLES, (GLsizei)surface_indices.size(), GL_UNSIGNED_INT, nullptr);
}
...
int main(void) {
	...
	cout << "Parametric Surface" << endl << endl;
	...
	cout << "Gyakorlat 1: Készítsünk egy félszabályos arkhimédészi testet a közelben!" << endl;
	cout << "Gyakorlat 2: Implementáljunk ütközés detektálást!" << endl;
	cout << "Gyakorlat 3: Szilárdtestmodellként világítsuk meg fényforrással őket!" << endl;
	cout << "Activity  1: Create a semi-regular polyhedra nearby!" << endl;
	cout << "Activity  2: Implement collison detection!" << endl;
	cout << "Activity  3: Make the models solid and apply a light source!" << endl;
